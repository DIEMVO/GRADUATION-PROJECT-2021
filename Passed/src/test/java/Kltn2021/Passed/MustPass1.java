...
//
		reportPath = "H:\\DataTest\\Report.html";
		ThreadContext.put("fileName", "H:\\DataTest\\Log.log");
		Log = LogManager.getLogger();		
		openBrowser();
		//headlessBrowser();
		check("H:\\DataTest\\primas-company-add.csv");
		closeBrowser();
	...
	public static void check(String val) throws Exception {
		...
		while ((csvCell = csvReader.readNext()) != null) {
			...
			// result
				...
				else if(csvCell[0].equals(s11) == true) {
					
					WebElement uploadGreeting = webDr.findElement(By.xpath(csvCell[1]));
					//send file from location into import file
					uploadGreeting.sendKeys(csvCell[2]);
					Thread.sleep(3000);	
...
//	public void importFile(WebDriver webDr, String file1) throws Exception {
//		String image = "Import";
//		WebElement uploadGreeting = webDr.findElement(By.xpath("//*[@id=\"file\"]"));
//		//send file from location into import file
//		uploadGreeting.sendKeys(links.Import + file1);
//		Thread.sleep(3000);	
//		webDr.findElement(By.id("btnUpload")).click();
//		Thread.sleep(1000);
//	
//	}
	...
	private static String processPattern(String pattern) {
		...
		try {
			...
			long occurance = pattern.chars().filter(new IntPredicate() {
				@Override
